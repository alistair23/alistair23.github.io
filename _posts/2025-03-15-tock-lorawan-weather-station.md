...
LoRaWAN works with a server and devices communicate with that server. With a similar setup to a WiFi router.
...
I2C soil moisture sensor, attached to the Qwiic bus: https://www.tindie.com/products/miceuz/i2c-soil-moisture-sensor/

### DFRoboto Rain Gauge

A tipping bucket rain gauge also connected via Qwiic: https://wiki.dfrobot.com/SKU_SEN0575_Gravity_Rainfall_Sensor

### SparkFun Environment Sensor

A temperature and humidity sensor, also attached via Qwiic: https://www.sparkfun.com/sparkfun-environmental-sensor-breakout-bme680-qwiic.html

### DFRobot Solar Manager and Solar Pannel

To power the device some LiPo batteries have been connected to a solar manager which can charge the batteries while powering the device. This allows the device to work without being externally powered and the battery is capable of running the monitor for multiple days.

https://www.dfrobot.com/product-2000.html

### Final product

Putting it all together with some batteries and in a water proof box results in this
...
There are full getting started steps in the [Tock repo](https://github.com/tock/tock/tree/master/boards/apollo3/lora_things_plus), but below is a quick start.
...
This guide is going to use Chirpstack. [The Things Network (TTN)](https://www.thethingsnetwork.org) will also work fine as a LoRaWAN server and it has been tested with the applications below. TTN can also integrate with Home Assistant and the steps should be similar.

Chirpstack ended up being easier to integrate with Home Assistant and works without any internet access.

Follow steps on the internet for creating an application and then a device under that application. You will need the Join EUI, Device EUI, Application Key and Network Key. Save these for the next step.
...
Upstream [libtock-c](https://github.com/tock/libtock-c) does support LoRa and LoRaWAN, but it is missing some key features as the community is reluctant to add functionality.

As such we are going to use a fork for the full functionality.
...
At this point you will need to edit the file `radioConfig.h` with values from your LoRaWAN server.
...
You can then run the application by flashing it to the board. To do this run `make flash-app` in the Tock `boards/apollo3/lora_things_plus` directory from above, but point to the application binary you just built.

```shell
APP=RadioLib/examples/NonArduino/Tock/build/tock-sx1261.tbf make flash-app
```

This application when run will store the LoRaWAN keys in the boards flash. That way our LoRaWAN application can load the values and use them.

### Running LoRaWAN sensor

Change to the libtock-c direction and then build the LoRaWAN application.
...
At this point the LoRaWAN application should be running and transmitting. You should be able to see the events on the server. It should look something like this. Note that the first connection will also include some join events.

![Chirpstack Events](/img/Apollo3-Tock-LoRa-Monitor-Chirpstack-Events.png "Chirpstack list of events")

## Setup weather station

That's it! At this point you have installed Tock and the LoRaWAN application. The application should be transmitting data which the server can decode.

If using TTN you should be able to see the measurements in the dashboard.

If using Chirpstack we will want to configure the measurements. Under "Device profiles" there is a "Measurements" tab. That hopefully will be auto populated, but if not it should look like this.
...
The LoRaWAN server should now ve receiving weather data from the device.
...
First is to install the Mosquitto broker if it isn't alredy setup. You can follow the guide here to set it up: https://github.com/home-assistant/addons/blob/master/mosquitto/DOCS.md

### Forward Chirspstack Data to MQTT

Next you will want to configure Chirpstack to use the MQTT server. I set this up using the SenseCAP [LoRa Network Configuration](https://wiki.seeedstudio.com/home_assistant_sensecap/#11-lora-network-configuration).
